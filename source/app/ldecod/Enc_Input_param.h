...
struct inp_par_enc
{
	...
	int qp[5];							  //!< QP values for all slice types
	int qp2frame;                         //!< frame in display order from which to apply the Change QP offsets
	int qp2off[5];						  //!< Change QP offset values for all slice types
...
#endif
	char outfile[255];					  //!< H.264 compressed output bitstream
	char ReconFile[255];				  //!< Reconstructed Pictures (view 0 for MVC profile)
	char ReconFile2[255];				  //!< Reconstructed Pictures (view 1)
	char TraceFile[255];				  //!< Trace Outputs
	char StatsFile[255];				  //!< Stats File
	char QmatrixFile[255];				  //!< Q matrix cfg file
	...
	char sp_output_filename[255];    //!<Filename where SP coefficients are output
	char sp2_input_filename1[255];   //!<Filename of coefficients of the first bitstream when encoding SP frames to switch bitstreams
	char sp2_input_filename2[255];   //!<Filenames of coefficients of the second bitstream when encoding SP frames to switch bitstreams
	...
	// FMO
	char SliceGroupConfigFileName[255];    //!< Filename for config info fot type 0, 2, 6
	...
	unsigned char *slice_group_id;                   //!< slice_group_id is for slice group type being 6
	...
	char QOffsetMatrixFile[255];        //!< Quantization Offset matrix cfg file
	...
	char ToneMappingFile[255];    //!< ToneMapping SEI message cfg file
